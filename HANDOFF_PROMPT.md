...
4. **研究→代码冷门收获（2026-09-07，随后真机修正）**：MoE 专家缓存 L2 淘汰策略做过一次探索——把 `choose_victim` 从 LRU 换成累积热计数（predictive eviction），原以为靠 k3 8 遍**单调超集**结构（pass_v⊆pass_v+1）能省 I/O（÷5.6）。**但 2026-09-08 真机证实该卖点不成立**：LFU 在 K3 trace 上退化、L2 容量+持久化才是主导。单调超集本身 92/92 层验证成立但要的不是淘汰算法。详见待办 10 修正块，勿引用 ÷5.6。
...
> - **首要结论（注意：未探到底）**：K3 部署 **L2 容量 + meta 持久化 >> 淘汰策略**——方向成立，但**具体容量拐点（多少 GB 全驻留）尚无真机数据**。真机 4-token 那次 100% 命中是**短会话个案**（恢复的热表恰好覆盖那 4 token），不可当普适结论；同批真机 trace 冷启动 sim 只到 128GB=64.31% 未封顶。fixtures 合成 trace 的"192GB=90%/10,010 专家全驻留"**来源未确认，不能当 K3 事实**。要定拐点须 PC 用 `--gen 32+` 长真机 trace 跑容量梯度。→ 故"100%/90%/192GB"均**待长 trace 验证**，勿引用为定论。
...
4. **K3 部署的真杠杆（替代淘汰算法之争）**：**L2 容量 + meta 持久化 >> 淘汰策略**。方向真机支持，但**容量拐点未探到底**——100% 只是短会话个案，fixtures 的 192GB/90% 来源未确认，需 `--gen 32+` 长 trace 验证（见待办10修正块）。
...
- [ ] **用 `--gen 32+` 长真机 trace 跑容量梯度，定位 K3 真实容量拐点（多少 GB 全驻留）**——当前 100%/90%/192GB 均未探到底（见待办10修正块）
