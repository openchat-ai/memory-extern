...
8. **「划时代」验证门槛（别停在结构数字好看，要端到端质量过关才算真）**：
   - 仅"唯一值少/二幂"不足以称公式化——那是**量化后的查表+移位加速**（有价值≠算出来的公式）。真公式化=权重能由极少量参数推导。
   - **决定性对照实验（桌面舰队跑，一步到位）：**
     a. 真实文本 **dense**（BF16 原始）跑 probe → 看**是否低秩且可重构**
     b. 真实 **routed expert**（MXFP4）跑 probe → 看**唯一值是否极少 ∧ 强二幂**
     c. **端到端质量测试（关键，最容易被跳过）**：低秩重构 dense → 注入 K3 跑**困惑度/激活误差**，误差近 0 才算"低秩可用"
   - 判断：仅 (a)(b) 数字好看 = "查表+移位加速 GEMV"，值得做但不是划时代；只有 (c) 质量几乎无损，才是"不须存全权重"的真创举。
   - 附：真实 BF16 dense 与 MXFP4-反量化 的唯一值差异本身就是关键信号（若 BF16 也极少唯一值 → 权重真有内在稀疏紧凑结构，非格式人造）。
