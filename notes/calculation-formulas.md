...
**⚠️ ¥1,174/颗 含 ¥8/GB 便宜内存假设（旧口径），实际 LPDDR5X ¥60/GB，见 `pim/calc_k3_shared_pool.py`。**
...
内存容量 = 8 × 16GB = 128 GB（独立内存上限，非实配）
SerDes = 4 lanes × 100 Gbps = 50 GB/s/chip
功耗 = 30W
成本 = ¥1,174（含封装+DRAM，旧假设 ¥8/GB）→ 真实 LPDDR5X ¥60/GB 时不含内存实配（见 calc_k3_shared_pool.py）
```
