...
**M86/M87 六角红队 (手动预验全捕, 全门被打断待轮)**: P2×F4、P2×F2、F2@P11 (o 族),
  F1@K16、F1@S4 (GEMM 族), F3@S9 (窗族)。单行 6/6 CAUGHT; 全量门于 100 行轮次被中断
  (仅落 94 行), 六行已入闸脚本待全门闭环。

**M88 另批 5 角**: HALF3×F3(窗族)、P4×F5×S2(GEMM)、P2×F1×V2048(GEMM t=8)、
  T280×F1(GEMM t=1 长程) 全捕; V512×FBPOLY2 PASS。门脚本 105 行; 全门闭环延至无中断
  窗口 (100/105 轮曾被中断)。
...
**M100 第六弹 5 角**: P0×F2×B2(o)、P0×F1×K5(GEMM t=1)、P3×F2(o)、P6×F1×H9(GEMM t=1) 全捕;
  **F1@B1×V512 独立复证 0PASS 掩蔽** (与 M85 同因: 注入点落从不计分切片; 掩蔽与 V512 无关)。

**M101 第七弹 5 角 (全捕, 待入闸)**: P11×F4×V2048(o)、P11×F1×V512(GEMM gw=12 远峰小窗)、
  P9×F2×V512(o)、P5×F3(窗界失配)、P8×F4×K12(o 三叠)。
...
**M112 门序守卫 (逆吃教训收编)**: 脚本预检查 `exit` 行号必须大于末行 `run_row` 行号,
  否则立即 exit 9 报「后列死码」。M90-补附表曾以 `cat >>` 追加在 exit 之后 (重复 M78 死码
  教训), 176 门日志自证只跑了 123 行 (54 角零行使), 由守卫根除同类复发。
...
**M117 覆盖矩阵全景账**: 门 200 行归一化谱 (184 元组):
