...
**trunk 压缩（bf16→MXFP4）不可行 —— 已在真实权重上实测验证。**
- 注意力类权重对精度敏感，低精度量化后质量崩坏，强行压缩不可用
- 我算的「trunk→~12GB、省 39%」是**纯纸面估算，未跑过真实权重量化，跑偏**
- **正确结论**：trunk 36GB 就是压不动的那部分，每 token 61.8GB 是现实底线
- **教训**：shape 重建/纸面省幅不能当结论，须先经真实权重验证；
  这也坐实了 k3-verdict §八「trunk 原始 110GB」是无法再压的 bf16 事实
- 不再在 trunk 压缩方向上投入
