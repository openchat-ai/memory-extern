...
⚠️ 此轮为 **L1=LRU**（默认策略）；同日二测补 heat，见下文"二测"段——结论随策略修正。
...
**结论（LRU 口径）：中位 107.27 vs 106.52，差 0.7%，远在 33% 噪声带内，判无效应。**
**此结论仅对 LRU 成立**——L1 换 heat 后同配置专家盘读 -19~32%、稳态 -6~10%（见下文二测）。

**为什么热专家落内存没提速（因果链，LRU 口径）**：
1. **LRU 无法让热集跨 token 驻留**——这是根本。LRU 按时间戳驱逐，刚 get 的专家下 token
   大概率被"本 token 最后用过的"挤掉；跨 token 复用靠运气而非热度。专家盘读恒 25.83GB
   （两档 cache 同值）即 LRU 保留失败的直接证据。
...
3. 稳态 ~105s 的瓶颈 = trunk 54GB 重读 + 专家 25.83GB NVMe 传输，两者都是物理必需字节量，
   cache 容量无关。
→ **二测修正：第 1 条是策略问题不是容量问题。** heat 让热集跨 token 留驻，专家盘读
   25.83→20.99GB（cache8）/17.44GB（cache15），稳态 96-100s。容量无效是 LRU 语义的产物。
...
**2026-09-17 二测（同日修正）：L1 替换策略才是杠杆，不是容量——heat 实测胜 LRU**

cache 8→15 无收益是 **LRU 语义局限**，不是"缓存容量无效"。给 L1 实现 heat
（K3_L1_POLICY=heat：驱逐"累计请求最少 且 本 token 未触碰"的专家，修复早期
实现会驱逐 in-use 专家导致输出全 0 的 bug——需 used_at recency 保护，见
kimi-k3-in-c `pick_victim` 注释）。gen=8 incremental cgroup 26GB n=3，输出逐
token 一致（一致性断言通过）：

| L1 策略 | cache | rep1 | rep2 | rep3 | 中位 | 专家盘读 |
|---|---|---|---|---|---|---|
| LRU | 8 | 186.97¹ | 107.27 | 102.00 | **107.27** | 25.83 GB |
| **heat** | 8 | 96.85 | 91.39 | 96.35 | **96.35** | **20.99 GB** |
| LRU | 15 | 118.91¹ | 101.83 | 106.52 | **106.52** | 25.83 GB |
| **heat** | 15 | 104.14¹ | 94.13 | 100.17 | **100.17** | **17.44 GB** |

¹ 冷启动离群。heat 冷启动损失显著更小（96.85 vs 186.97），因热集跨 token 保留更有效。

**结论**：
1. **heat 稳态胜 LRU 6-10%**（cache8 中位 96.35 vs 107.27；cache15 100.17 vs 106.52）。
2. **heat 减少专家盘读 19-32%**（cache8 25.83→20.99GB；cache15 25.83→17.44GB）——
   热集跨 token 留在 RAM 生效，正是论文判据"复用落最快层"的正面应用。
3. **容量本身非杠杆**（heat 下 cache8≈cache15），因 trunk 54GB/step 重读成为稳态墙。
4. 早期"L1 heat 破坏正确性"结论错误，根因是实现 bug（无 recency 保护驱逐 in-use 专家）；
   修正后 heat 正确且更优。**判据正面落地：内存 heat 保留热集 → 慢层读取下降。**
