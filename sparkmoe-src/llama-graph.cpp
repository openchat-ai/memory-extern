...
llm_graph_context::llm_graph_context(const llm_graph_params & params) :
    ...
    n_expert_used    (cparams.moe_paging.mode == LLAMA_MOE_PAGING_MODE_EXPLICIT ?
                      hparams.n_expert_used : (cparams.warmup ? hparams.n_expert : hparams.n_expert_used)),
...
ggml_tensor * llm_graph_context::build_lora_mm_id(
          ...
          ggml_tensor * ids,
          ggml_tensor * w_s,
          ggml_tensor * auxiliary_ids) const {
    ggml_tensor * res = ggml_mul_mat_id(ctx0, w, cur, ids);

    ggml_tensor * side_ids = auxiliary_ids ? auxiliary_ids : ids;
    ...
    if (w_s) {
        ...
        s = ggml_get_rows(ctx0, s, side_ids);
    ...
    for (const auto & lora : *loras) {
        ...
        ggml_tensor * ab_cur = ggml_mul_mat_id(
                ctx0, lw->b,
                ggml_mul_mat_id(ctx0, lw->a, cur, side_ids),
                side_ids
                );
...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    ggml_tensor * gate_up_exps_source = gate_up_exps;
    ggml_tensor * up_exps_source = up_exps;
    ggml_tensor * gate_exps_source = gate_exps;
    ggml_tensor * down_exps_source = down_exps;

    if (moe_paging && il >= 0 && moe_paging->handles_layer(il)) {
        gate_up_exps_source = moe_paging->bind_pool(il, gate_up_exps);
        up_exps_source = moe_paging->bind_pool(il, up_exps);
        gate_exps_source = moe_paging->bind_pool(il, gate_exps);
        down_exps_source = moe_paging->bind_pool(il, down_exps);

        auto * contiguous_ids = ggml_cont(ctx0, selected_experts);
        routing_ids = moe_paging->remap_ids(ctx0, contiguous_ids, il);
        cb(routing_ids, "ffn_moe_slot_ids", il);
    }

    if (gate_up_exps) {
        // merged gate_up path: one mul_mat_id, then split into gate and up views
        ggml_tensor * gate_up = build_lora_mm_id(gate_up_exps_source, cur, routing_ids, up_exps_s, selected_experts); // [n_ff*2, n_expert_used, n_tokens]
        ...
        up = build_lora_mm_id(up_exps_source, cur, routing_ids, up_exps_s, selected_experts); // [n_ff, n_expert_used, n_tokens]
        cb(up, "ffn_moe_up", il);
        ...
        if (gate_exps) {
            cur = build_lora_mm_id(gate_exps_source, cur, routing_ids, gate_exps_s, selected_experts); // [n_ff, n_expert_used, n_tokens]
            cb(cur, "ffn_moe_gate", il);
        } else {
            cur = up;
        }
    ...
    experts = build_lora_mm_id(down_exps_source, cur, routing_ids, down_exps_s, selected_experts); // [n_embd, n_expert_used, n_tokens]
