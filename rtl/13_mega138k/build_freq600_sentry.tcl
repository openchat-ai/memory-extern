# build_freq600_sentry.tcl — 600 哨兵 PnR（PLL-X600 实体 + 哨兵计数/诊断）
# 用途：800 SENTRY-FAIL 疑受 FBDIV 配置 bug 污染后的对照——600 合法参数化
#       (VCO 1200MHz) 应可锁可收敛, 反证"板子/工具链无问题, 800 参数不合法"。
# 产物: out/138k_pro/freq600_sentry
# 用法: gw_sh build_freq600_sentry.tcl [route_option] [place_option] [max_fanout]
# ============================================================================

set TOP board_freq600_sentry
set SRC  F:/sram/sram/rtl/13_mega138k
set OUT  F:/sram/sram/out/138k_pro/freq600_sentry
...
add_file $SRC/gowin_pll_x600.v
add_file $SRC/freq600_sentry.sdc
add_file $SRC/freq600_sentry.cst
...
puts "=== FREQ600_SENTRY: route=${ROUTE_OPT} place=${PLACE_OPT} max_fanout=${MAX_FANOUT} (PLL-X600 + 600 域哨兵) ==="
...
set result_file "$SRC/freq600_sentry_result.txt"
set fp [open $result_file w]
puts $fp "# freq600_sentry PnR 结果（600 哨兵，PLL-X600 + fabric 1.667ns 收敛判定）"
...
puts "=== FREQ600_SENTRY DONE (route=${ROUTE_OPT} place=${PLACE_OPT} max_fanout=${MAX_FANOUT} elapsed ${elapsed}s) ==="
