...
//----------------------------------------------------------------------------
// attn_inner_ctl_sf.v — SF3-P3 综合镜像 (单差异: q_vec 端口打包)
//
// 与 41/attn_inner_ctl.v 逐行同步 (唯一差异 = UNPACKED 数组端口 → packed 标量,
// termux 侧 yosys 0.68 不支持数组端口)。产品源码/验证 tb 零改动; 本镜像仅供
// synth_gowin 求资源账。改产品版时必须同步本文件; 上板联调前将接口统一回本版。
//
...
module attn_inner_ctl_sf #(
    ...
    // SF3-P3: packed 打包 (yosys 0.68 不支持数组端口), 下方生成切片
    input  wire [32*WPR-1:0]  q_vec_p,
    ...
    // SF3-P3: 打包端口切片 → 原样 q_vec 数组 (综合镜像专用)
    wire [15:0] q_vec [0:FEED-1];
    genvar gs;
    generate for (gs = 0; gs < FEED; gs = gs + 1) begin : QSL
        assign q_vec[gs] = q_vec_p[16*gs +: 16];
    end endgenerate
