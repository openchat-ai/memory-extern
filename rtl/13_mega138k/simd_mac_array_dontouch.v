    ...
    generate
        ...
        for (i = 0; i < NUM_LANES; i = i + 1) begin : gen_lane
            ...
            if (PIPE_MUL == 0) begin : gen_combo
                ...
                reg  kx_vld /* synthesis syn_dont_touch = 1 */;                          // 上一拍 kx 有效
                reg  signed [11:0] kx_pipe /* synthesis syn_dont_touch = 1 */;           // Stage A：锁存 kx
                reg  kx_sgn /* synthesis syn_dont_touch = 1 */;                          // Stage A：锁存符号（与 kx 同拍）
                always @(posedge clk) begin
                    if (!rst_n) begin
                        kx_vld  <= 1'b0;
                        kx_pipe <= 12'sd0;
                ...
                wire signed [12:0] prod13_p =
                    kx_sgn ? -$signed({kx_pipe, 1'b0})
                           :  $signed({kx_pipe, 1'b0});
                reg  signed [ACC_WIDTH-1:0] prod_pipe0 /* synthesis syn_dont_touch = 1 */;
                always @(posedge clk) begin
                    if (!rst_n)
                ...
                reg  prod_vld /* synthesis syn_dont_touch = 1 */;
                always @(posedge clk) begin
