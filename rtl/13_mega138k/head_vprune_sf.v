...
module head_vprune_sf #(
    ...
    // SF3-P2: VP 镜像打包连接 (yosys 0.68 无数组端口)
    localparam CW2 = $clog2(VOC);
    wire [CAP*CW2-1:0] vp_cand_p;
    vocab_prune_sf #(.VOC(VOC), .GRP(GRP), .BB(BB), .MAXE(MAXE), .FKXG(FKXG)) VP(
        .acc(acc), .xext(xext), .G(g_w), .ncad(ncad_w), .cand_p(vp_cand_p)
    );
    genvar gcs;
    generate for (gcs = 0; gcs < CAP; gcs = gcs + 1) begin : CSL
        assign cand_w[gcs] = vp_cand_p[gcs*CW2 +: CW2];
    end endgenerate
    assign peak_g = g_w;
    ...
    output_head_sf #(.TN(1), .VOC(CAP), .K(K), .BB(BB), .RNDW(RNDW)) OH(
