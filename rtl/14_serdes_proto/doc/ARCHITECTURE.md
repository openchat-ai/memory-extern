...
- 目标: 把"扫描目录/找文件→LBA"也搬进 RTL(宿主工具定位结果作参照)。**已完成阶段1-8**:
  ...
  **阶段3/4** 循根目录首 extent 指向的数据块, 用**字节游标(dpos)按 rec_len 逐条遍历**
  ...
  未命中则 qvalid=0)——即"文件 inode→物理块段"的可查询映射, 正是 `file2lba` 的"文件→LBA"
  查询在 RTL 的落点。→"文件名→inode→extent→物理块"闭合链完整, 与 `file2lba`/`ext4_lba.py`
  对齐。
  后续按序扩展: 全文件 **extent 深度递归**(>0 也收集, 而非跳过)→多目录递归→大表(多组/
  多块 inode 表, 非固定块4)→cache 管线接入查询。
...
| 18 | ext4_scan | RTL ext4 扫描: 阶段1 superblock+组0desc → 阶段2 根inode(2) → 阶段3/4 目录块rec_len逐条枚举→结果表 → 阶段7 全文件遍历(动态 inode word)→"文件→物理块"映射表 → 阶段8 落外置表RAM+独立查询FSM(按ino查询返回物理块段, 未命中qvalid=0) |
