...
#  18. ext4_scan        RTL ext4 扫描: 阶段1 superblock+组0desc → 阶段2 根inode → 阶段3/4 目录rec_len枚举→结果表 → 阶段7 全文件收集 → 阶段9/10 索引多层递归下钻多叶 → 阶段12 索引块多路遍历(全部ei_leaf) → 阶段11 一级子目录递归 → 阶段8 落表RAM+查询FSM(合成镜像闭环)
