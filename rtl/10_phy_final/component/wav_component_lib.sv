...
`endif
   ...
   // ------------------------------------------------------------------
   // Simulation-only TCM firmware auto-load (sim convenience).
   // TB passes +RAMDIR=<dir>; the filename is derived from the instance
   // hierarchy:  ...u_itcm.tcm_sram[<bank>].tcm_32bit.tcm_sram  becomes
   //   <dir>/itcm_2048x4_tcm0_b<bank>_byte03_byte00.ram
   // No-op unless the plusarg is given and geometry matches a TCM bank.
   // NOTE: cannot use `ifndef SYNTHESIS here -- ddr_global_define.vh
   // defines SYNTHESIS unconditionally in this project!
   // ------------------------------------------------------------------
`ifndef WDDR_NO_RAMLOAD
   initial begin
      string dir;
      if ($value$plusargs("RAMDIR=%s", dir) && DWIDTH == 32 && SIZE == 2048) begin
         string hp, pre, fname;
         int upos, bpos, bi, i;
         byte c;
         hp = $sformatf("%m");
         pre = "itcm";
         upos = -1; bpos = -1; bi = 0;
         for (i = 0; i+6 <= hp.len(); i++) begin
            if (hp.substr(i, i+5) == "u_dtcm") begin pre = "dtcm"; upos = i+7; break; end
            if (hp.substr(i, i+5) == "u_itcm") begin upos = i+7; break; end
         end
         if (upos >= 0)
            for (i = upos; i+9 <= hp.len(); i++)
               if (hp.substr(i, i+8) == "tcm_sram[") begin bpos = i+9; break; end
         if (bpos >= 0)
            forever begin
               c = hp[bpos];
               if (c >= "0" && c <= "9") begin
         ...
         fname = $sformatf("%0s/%0s_2048x4_tcm0_b%0d_byte03_byte00.ram", dir, pre, bi);
         if (upos >= 0 && bpos >= 0) begin
            $readmemb(fname, mem);
            $display("[TCMLOAD] %0s <- %0s", hp, fname);
         end
      end
   end
`endif
