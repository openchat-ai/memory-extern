...
module ddr_fc_dly #(
   ...
   genvar i ;
   genvar b ;
   // Lint tool compatibility: expanded SV array instances into explicit loops
   // (Verilator cannot handle per-bit array instantiation with non-wire outputs)
   generate
      for (i = MAXDLY-1; i >= 0 ; i-- ) begin : PIPE
         for (b = 0; b < DWIDTH ; b++ ) begin : BITS
            if(i== MAXDLY-1) begin: PIPE1
               ddr_dff u_dff (.i_clk(i_clk), .i_d(i_d[b]), .o_q(q[i][b]));
            end else begin: PIPE
               ddr_dff u_dff (.i_clk(i_clk), .i_d(d[i][b]), .o_q(q[i][b]));
            end
         end
      end
      for (i = MAXDLY-1; i > 0 ; i-- ) begin : MUX
         for (b = 0; b < DWIDTH ; b++ ) begin : BITS
            ddr_mux u_mux (.i_sel(dly[i]), .i_a(i_d[b]), .i_b(q[i][b]) , .o_z(d[i-1][b]));
         end
      end
   endgenerate
