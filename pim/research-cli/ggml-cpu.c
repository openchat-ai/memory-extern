...
#if defined(_MSC_VER) || defined(__MINGW32__)
    return _aligned_realloc(ptr, new_cap, 64);
#else
    if (ptr == NULL) {
        void * p = NULL;
...
void ggml_chip_set_threads(int n) {
    ...
    if (n > 0 && ggml_chip_nthreads == 0) {
        for (int i = 0; i < GGML_CHIP_MAX_THREADS; i++) { sem_init(&ggml_chip_sem_submit[i], 0, 0); }
        ...
        atomic_init(&ggml_chip_copy_ns_total, 0);
        atomic_init(&ggml_chip_copy_bytes_total, 0);
        atomic_init(&ggml_chip_compute_ns_total, 0);
        for (int i = 0; i < n; i++) {
...
void ggml_chip_set_sync_mode(int m) {
    atomic_store_explicit(&ggml_chip_sync_mode, m, memory_order_relaxed);
}

void ggml_chip_set_copy_only(int v) {
    atomic_store_explicit(&ggml_chip_copy_only, v, memory_order_relaxed);
}

void ggml_chip_set_nt_copy(int v) {
    atomic_store_explicit(&ggml_chip_nt_copy, v, memory_order_relaxed);
}
...
void ggml_chip_get_breakdown(int64_t * copy_ns, int64_t * copy_bytes, int64_t * compute_ns) {
    *copy_ns     = atomic_load_explicit(&ggml_chip_copy_ns_total, memory_order_relaxed);
    *copy_bytes  = atomic_load_explicit(&ggml_chip_copy_bytes_total, memory_order_relaxed);
    *compute_ns  = atomic_load_explicit(&ggml_chip_compute_ns_total, memory_order_relaxed);
}
